...
export const deleteProduct = async (req, res) => {
    try {
        ...
        res.json({message: "Proudct deleted successfully"});
    } catch (error) {
...
export const getProductsByCategory = async (req, res) => {
    ...
    try {
        ...
        console.log("Error in getproduct by category controller", error.message);
        res.status(500).json({message: "server error", error: error.message});
    }
};
...
async function updateFeaturedProductCache() {
    try {
        ...
        await redis.set("featured_products", JSON.stringyfy(featuredProducts));
    }catch(error) {
